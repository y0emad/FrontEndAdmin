...
export function NavBarMain() {
  ...
  return (
    ...
    >
      <div
        className={`nav-links me-auto lg:m-3 lg:static  lg:min-h-fit lg:items-center absolute bg-[#000915] ${
          token ? "min-h-[25vh]" : "min-h-[5vh]"
        } bottom-[240%] grid  left-0 lg:w-auto lg:py-0  w-full  text-gray-200 fontMed `}
      >
      ...
      <div className=" grid lg:grid-cols-2  grid-cols-1 h-33 m-3  text-gray-200 ms-auto  gap-5  items-center justify-center">
        ...
        {token ? (
          <>
            <span
              onClick={logOut}
              className="lg:flex  gap-2 justify-center  hover:text-[#7f6727] text-nowrap  items-center fontMed hidden cursor-pointer"
            >
              <span>{t("header.Log_out")}</span>
              <LogoutOutlined />
            </span>
